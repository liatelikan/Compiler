...
module Tag_Parser : TAG_PARSER = struct
  ...
  let rec macro_expand_qq = function
    ...
    | ScmPair (ScmPair (ScmSymbol "unquote-splicing",
                        ScmPair (sexpr, ScmNil)), rest) ->
        ScmPair (ScmSymbol "append",
          ScmPair (sexpr,
          ScmPair (macro_expand_qq rest, ScmNil)))
    | ScmPair (a, rest) ->
        ScmPair (ScmSymbol "cons",
          ScmPair (macro_expand_qq a,
          ScmPair (macro_expand_qq rest, ScmNil)))
    | ScmVector sexprs ->
        let schemeList = List.fold_right (fun a b -> ScmPair (a, b)) sexprs ScmNil in
        ScmPair (ScmSymbol "list->vector",
          ScmPair (macro_expand_qq schemeList, ScmNil))
    | sexpr -> ScmPair (ScmSymbol "quote" , ScmPair(sexpr, ScmNil));;
  ...
  let rec macro_expand_cond_ribs ribs =
    ...
    | ScmPair (ScmPair (ScmSymbol "else", body), _rest) ->
        ScmPair (ScmSymbol "begin", body)

    | ScmPair ( ScmPair (test,
                        ScmPair (ScmSymbol "=>",
                                  ScmPair (fexpr, ScmNil))),
                rest_ribs) ->
        let rest_expanded = macro_expand_cond_ribs rest_ribs in
        ...
        ScmPair
          (ScmSymbol "let",
          ScmPair (bindings, ScmPair (if_expr, ScmNil)))

    | ScmPair (ScmPair (test, body), rest_ribs) ->
        let rest_expanded = macro_expand_cond_ribs rest_ribs in
        let then_expr = ScmPair (ScmSymbol "begin", body) in
        ScmPair
          (ScmSymbol "if",
          ScmPair
            (test,
    ...
    | _ ->
        raise (X_syntax "Illegal cond-rib")
...
module Semantic_Analysis : SEMANTIC_ANALYSIS = struct
  ...
  let annotate_lexical_address =
    let rec run expr params env =
      ...
      | ScmLambda (params', Opt opt, expr) ->
         let optional_params = List.append params' [opt] in
         ScmLambda' (params', Opt opt, run expr optional_params (params :: env))
      | ScmApplic (proc, args) ->
  ...
  let code_gen exprs' =
    ...
    let rec run params env = function
      ...
      | ScmApplic' (proc, args, Tail_Call) -> 
         let label_copy_stack = make_tc_applic_recycle_frame_loop ()
         and label_copy_stack_end = make_tc_applic_recycle_frame_done () in
         let args_code =
           String.concat ""
             (List.map
                 (fun arg ->
         ...
         ^ "\tmov rbp, qword[rbp]\n"
        (*fix stack*)
         ^ "\tmov rcx, qword [rsp + 8 * 2] \t\t ; M = size of B\n"
         ^ "\tlea rsi, [rsp + 8 * (2 + rcx)] \t ; last arg in B\n"
         ^ "\tmov rdx, qword [rsp+ 8 * (2 + rcx + 4)] \t ; N = size of A\n"
         ^ "\tadd rdx,rcx \n"
         ^ "\tlea rdi, qword [rsp + 8 * (2 + 4 + rdx)] \t\t ; last arg in A\n"
         ^ "\tadd rcx, 3 \t\t\t\t ; rcx = size of B + 4 (M, env, ret,rbp)\n"
         ^ (Printf.sprintf "%s:\n" label_copy_stack)
         ^ "\tmov rbx, qword [rsi]\n"
         ^ "\tmov qword [rdi], rbx\n"
         ^ "\tdec rcx\n"
         ^ "\tcmp rcx, 0\n"
         ^ (Printf.sprintf "\tje %s\n" label_copy_stack_end)
         ^ "\tsub rsi, 8*1\n"
         ^ "\tsub rdi, 8*1\n"
         ^ (Printf.sprintf "\tjmp %s\n" label_copy_stack)
         ^ (Printf.sprintf "%s:\n" label_copy_stack_end)
