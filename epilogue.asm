...
;;; r8 : params
;;; r9 : | env |
;;; r8 : number of params in current function
;;; r9 : env depth
...
.L_rib_size_ok:
        ...
        .L_copy_params_loop:
        cmp rsi, r10              
        je .L_copy_params_done

        mov rdx, qword [r11 + 8*4 + 8*rsi]
        mov qword [rbx + 8*rsi], rdx
        
        inc rsi
        jmp .L_copy_params_loop
...
L_code_ptr_bin_apply:
        cmp  qword [rsp + 8 * 2], 2   ;check number of args 
        jne  L_error_arg_count_2
        mov  r9, qword [rsp + 8 * 3]  ; f 
        cmp  byte[r9], T_closure          ;check if closure   
        jne  L_error_non_closure
        mov  r8, qword [rsp]        ; ret
        mov  r10, qword [rsp + 8 * 4] ; s 
        lea  rbx, qword [rsp + 8 * 4] ; top stack 
        mov  rcx, 0                 ;counter 
.L0:
        cmp  byte [r10], T_nil     ; if end of list 
        je   .L0_out
        cmp  byte [r10], T_pair 
        jne  L_error_improper_list  ; if pair 
        mov  rdx, qword [r10 + 1]    ; car
        mov  qword[rbx], rdx 
        mov  r10, qword [r10 + 1 + 8] ; cdr 
        sub  rbx, 8 * 1
        inc  rcx
        jmp  .L0
.L0_out:
        mov  qword [rbx], rcx     ; enter n 
        sub  rbx, 8 * 1
        mov  rdx, qword [r9 + 1]
        mov  qword [rbx], rdx      ; enter env f 
        sub  rbx, 8 * 1 
        mov  qword [rbx], r8       ;enter ret 
        mov  rsp, rbx 
        lea  rsi, [rsp + 8 * rcx + 8 * 2]  ; point at top of stack 
        lea  rdi, [rsp + 8 *3]             ; point to the first arg 
.L1:                                       ; switch the order of arg
        cmp  rsi, rdi 
        jle  .L1_out 
        mov  rbx, qword [rsi]
        xchg rbx, qword [rdi]
